  ...
  return (
    <div className="pt-8 sm:pt-12 lg:pt-[60px] relative">
      <div className="w-full h-[300px] sm:h-[500px] lg:h-[850px] relative ">
        ...
        />
        <div className="px-4 sm:px-6 lg:px-0 mt-8 lg:mt-0 lg:absolute lg:top-[50px] lg:left-[80px] flex flex-col items-start justify-center leading-tight">
          <motion.span
            initial={{ opacity: 0, x: -40 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5 }}
            className="text-[#706d68] text-3xl sm:text-4xl md:text-5xl lg:text-(length:--text-h2-base,4.25rem) font-sans leading-heading font-extralight lg:tracking-[-2px]"
          >
            Data so accurate
          </motion.span>
          <motion.em
        ...
        <div className="px-4 sm:px-6 lg:px-0 mt-6 lg:mt-0 lg:absolute lg:bottom-[50px] lg:left-[80px]">
          <motion.p
            initial={{ opacity: 0, x: -40 }}
            animate={{ opacity: 1, x: 0 }}
        ...
        <div className="px-4 sm:px-6 lg:px-0 mt-8 lg:mt-0 lg:absolute lg:right-[80px] lg:top-[50px] grid grid-cols-1 sm:grid-cols-2 lg:flex lg:flex-col gap-4 lg:gap-7">
          {statsData.map((item, index) => (
            <motion.div
              ...
              className="flex flex-row gap-x-6 justify-between items-start p-5 sm:p-7 bg-[#1a1918]/50 rounded-lg"
            >
              <div className="flex flex-col gap-1">
                <span className="text-3xl sm:text-4xl lg:text-5xl text-[#F7F1E8] font-serif leading-tight font-extralight">
                  {item.value}
                </span>
                <span className="text-sm text-[#F7F1E8] font-extralight">
                  {item.title}
                </span>
